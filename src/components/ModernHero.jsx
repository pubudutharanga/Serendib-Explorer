...
export default function ModernHero() {
  ...
  // Resolve & preload images using Vite base URL so paths include the correct prefix on GH Pages
  useEffect(() => {
    const checkLocalImages = async () => {
      // import.meta.env.BASE_URL is set by Vite; use './' fallback for dev convenience
      const base = import.meta.env.BASE_URL ?? './'
      const localUrls = bgPaths.map(p => {
        // ensure we don't produce double slashes if base ends with '/'
      ...
      for (let i = 0; i < localUrls.length; i++) {
        const src = localUrls[i]
        try {
          // HEAD is lighter; 'cache: no-store' avoids stale service worker cache
          const res = await fetch(src, { method: 'HEAD', cache: 'no-store' })
          if (res.ok) {
            availableImages.push(src)
          } else {
            availableImages.push(fallbackBackgrounds[i % fallbackBackgrounds.length])
          }
        } catch (err) {
          availableImages.push(fallbackBackgrounds[i % fallbackBackgrounds.length])
        }
      ...
      // Preload images (mark loaded when all complete)
      let loadedCount = 0
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-cyan-900">
      ...
      <div className="relative z-20 w-full max-w-6xl mx-auto px-4">
        ...
        >
          <div className="space-y-8">
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 pt-6">
              <Link to="/trending" className="group">
                <ModernButton size="lg" className="relative overflow-hidden bg-gradient-to-r from-cyan-500 to-blue-600 border-0 shadow-xl hover:shadow-2xl transition-all duration-300 min-w-[200px]">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-8">
              {[
                ...
                <motion.div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 backdrop-blur-lg border border-white/10 hover:bg-white/10 transition-all duration-300 group" whileHover={{ scale: 1.02, y: -2 }} transition={{ delay: index * 0.1 }}>
          ...
          <motion.div variants={containerVariants} className="grid grid-cols-2 gap-4 relative">
            <motion.div variants={floatVariants} animate="float" className="col-span-2 bg-white/10 backdrop-blur-2xl rounded-2xl border border-white/20 p-6 shadow-xl hover:shadow-2xl transition-all duration-500 group cursor-pointer overflow-hidden" whileHover={{ y: -4, transition: { duration: 0.3 } }}>
              ...
              <div className="relative z-10 space-y-4">
                ...
                <p className="text-white/70 text-sm leading-relaxed">UNESCO World Heritage site with ancient palace ruins and breathtaking views.</p>
            ...
            <motion.div variants={floatVariants} animate="float" transition={{ delay: 0.5 }} className="bg-white/10 backdrop-blur-2xl rounded-2xl border border-white/20 p-4 shadow-lg hover:shadow-xl transition-all duration-500 group cursor-pointer overflow-hidden" whileHover={{ y: -2, transition: { duration: 0.3 } }}>
            ...
            <motion.div variants={floatVariants} animate="float" transition={{ delay: 1 }} className="bg-white/10 backdrop-blur-2xl rounded-2xl border border-white/20 p-4 shadow-lg hover:shadow-xl transition-all duration-500 group cursor-pointer overflow-hidden" whileHover={{ y: -2, transition: { duration: 0.3 } }}>
            ...
            <div className="absolute -inset-4 -z-10">
              <motion.div className="absolute top-1/2 left-1/2 w-48 h-48 bg-cyan-400/5 rounded-full blur-2xl" animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} />
            </div>
          </motion.div>
        </motion.div>
      </div>
